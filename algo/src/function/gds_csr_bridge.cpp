...
// The fallback: materialize the undirected adjacency by scanning storage, exactly as the GDS
// functions did before the zero-copy path existed.
...
BridgeCSR buildUndirectedCSR(main::ClientContext* context, const std::string& graphName,
    ...
    auto materialized = fromMaterializedCsr(context, graphName, numNodes);
