...
constexpr const char* EXTRACT_SYSTEM_PROMPT =
    ...
    "instructions. Follow the extraction instruction. Return only the extracted answer and no explanation.";
...
struct AIExtractBindData final : FunctionBindData {
    ...
    AIExtractBindData(std::vector<LogicalType> paramTypes, std::string apiKey, std::string provider,
        ...
        : FunctionBindData{std::move(paramTypes), LogicalType::STRING()}, apiKey{std::move(apiKey)},
          ...
          model{std::move(model)}, endpoint{std::move(endpoint)} {}

    std::unique_ptr<FunctionBindData> copy() const override {
        return std::make_unique<AIExtractBindData>(LogicalType::copy(paramTypes), apiKey, provider, model, endpoint);
    }
};
...
public:
    ExtractTask(uint64_t maxThreads, std::vector<ExtractJob>& jobs, const AIExtractBindData& bindData,
        main::ClientContext* clientContext, std::string apiKey)
        : Task{maxThreads}, jobs{jobs}, bindData{bindData}, clientContext{clientContext}, apiKey{std::move(apiKey)} {}
    ...
    void run() override {
        ...
        while (!stop.load()) {
            ...
            try {
                ...
                CompletionRequest request{EXTRACT_SYSTEM_PROMPT, buildUserPrompt(job.text, job.instruction),
                    bindData.model, bindData.endpoint, apiKey, clientContext};
...
void validateStringArgument(const ScalarBindFuncInput& input, uint64_t index) {
    ...
    if (type != LogicalType::STRING()) {
        throw BinderException(std::format("{} argument {} must be STRING.", AIExtract::name, index + 1));
    }
}

std::string readLiteralString(const ScalarBindFuncInput& input, uint64_t index) {
    validateStringArgument(input, index);
    auto expression = ExpressionUtil::applyImplicitCastingIfNecessary(input.context, input.arguments[index],
        LogicalType::STRING());
    ...
    return ExpressionUtil::evaluateLiteral<std::string>(input.context, expression, LogicalType::STRING());
}

void validateEndpoint(const std::string& endpoint) {
    if (endpoint.empty() || (endpoint.rfind("http://", 0) != 0 && endpoint.rfind("https://", 0) != 0) ||
...
std::unique_ptr<FunctionBindData> bindFunc(const ScalarBindFuncInput& input) {
    if (input.arguments.size() < 3 || input.arguments.size() > 6) {
        throw BinderException(std::format("{} expects between 3 and 6 STRING arguments.", AIExtract::name));
    }
...
uint64_t parameterPos(const std::shared_ptr<ValueVector>& parameter, const SelectionVector* selection,
    sel_t selectedIndex) {
...
void execFunc(const std::vector<std::shared_ptr<ValueVector>>& parameters,
    ...
    for (auto selectedIndex = 0u; selectedIndex < resultSelVector->getSelSize(); ++selectedIndex) {
        ...
        const auto instructionPos = parameterPos(parameters[1], parameterSelVectors[1], selectedIndex);
    ...
    TaskScheduler::Get(*clientContext)->scheduleTaskAndWaitOrError(
        task, clientContext, true /* launchNewWorkerThread */);
...
function_set AIExtract::getFunctionSet() {
    ...
    for (uint64_t argumentCount = 3; argumentCount <= 6; ++argumentCount) {
        auto function = std::make_unique<ScalarFunction>(name,
            std::vector<LogicalTypeID>(argumentCount, LogicalTypeID::STRING), LogicalTypeID::STRING, execFunc);
